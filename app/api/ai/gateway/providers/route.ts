...
export async function GET(req: NextRequest) {
  ...
  if (process.env.ANTHROPIC_API_KEY) {
    const t = Date.now();
    try {
      const r = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'x-api-key': process.env.ANTHROPIC_API_KEY!, 'anthropic-version': '2023-06-01', 'content-type': 'application/json' }, body: JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 1, messages: [{ role: 'user', content: 'ping' }] }) });
      providers.push({ id: 'anthropic', name: 'Anthropic Claude', available: r.ok, latency_ms: Date.now() - t, primary: true });
    } catch { providers.push({ id: 'anthropic', name: 'Anthropic Claude', available: false, latency_ms: null, primary: true }); }
  } else {
  ...
  if (process.env.OPENROUTER_API_KEY) {
    ...
    } catch { providers.push({ id: 'openrouter', name: 'OpenRouter', available: false, latency_ms: null, primary: false }); }
  } else {
