...
const KEY_FIELDS = ['openrouter_key'] as const;
...
export async function POST(request: NextRequest) {
  ...
  if (!KEY_FIELDS.includes(field)) return NextResponse.json({ error: 'Invalid field' }, { status: 400 });
  if (typeof value !== 'string' || !value.trim()) return NextResponse.json({ error: 'Value required' }, { status: 400 });
...
export async function DELETE(request: NextRequest) {
  ...
  if (!KEY_FIELDS.includes(field as KeyField)) return NextResponse.json({ error: 'Invalid field' }, { status: 400 });
