  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: typeof window !== 'undefined' ? `${window.location.origin}/today` : '',
        }
      });
                    ...
                    <form action={loginAction} className="space-y-4">
                      ...
                      <div className="relative my-3 flex items-center justify-center">
                        <div className="absolute inset-0 flex items-center">
                          <div className="w-full border-t border-[#e6e5e0]"></div>
                        </div>
                        <span className="relative bg-white px-2.5 text-[9px] font-bold uppercase tracking-wider text-[#807d72]">ou</span>
                      </div>
                      ...
                      <button
                        ...
                        disabled={isPending}
                        className="w-full rounded-full bg-white hover:bg-neutral-50 border border-[#e6e5e0] text-[#26251e] py-2.5 text-xs font-bold transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60"
                      >
