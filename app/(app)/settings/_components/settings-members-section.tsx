    ...
    >
      ...
      <div className="border border-border rounded-xl p-5 bg-card space-y-3">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-bold text-foreground">Aperçu des membres</p>
            <p className="text-[11px] text-muted-foreground mt-0.5">Membres actifs dans cet espace de travail.</p>
          </div>
        </div>
        <div className="flex items-end gap-1">
          <span className="text-3xl font-bold text-foreground leading-none">{members.length}</span>
          <span className="text-[11px] text-muted-foreground mb-0.5">membre(s)</span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-lg text-xs font-bold hover:bg-primary/90 transition-colors"
          >
        ...
        {showInvite && (
          <div className="border border-border rounded-xl p-4 bg-card space-y-3 animate-in fade-in duration-150">
            <p className="text-xs font-bold text-foreground">Inviter par email</p>
        ...
        <div className="border border-border rounded-xl overflow-hidden bg-card">
          {/* Table header */}
          <div className="grid grid-cols-[1fr_120px_120px_40px] gap-2 px-4 py-2.5 border-b border-border bg-muted/40">
            <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Nom</span>
            <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Rôle</span>
            <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Plan</span>
            <span />
          </div>
          ...
          {loading ? (
            ...
            <div className="px-4 py-8 text-center text-xs text-muted-foreground">Aucun membre trouvé.</div>
          ) : (
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-7 h-7 rounded-full bg-primary/10 flex items-center justify-center shrink-0 overflow-hidden">
                      {m.avatarBase64 ? (
                        ...
                        <span className="text-[11px] font-bold text-primary">{initials(m)}</span>
                      )}
                    </div>
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-foreground truncate">{m.full_name || 'Utilisateur'}</p>
                      <p className="text-[10px] text-muted-foreground truncate">{m.email}</p>
                    </div>
                  </div>
                  <span className="text-xs text-foreground capitalize">{m.role || 'Membre'}</span>
