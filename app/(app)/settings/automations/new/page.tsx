...
export default function NewAutomationPage() {
  ...
  const ACTIONS = useMemo(() => {
    return [
      ...
      { key: 'send_email' as Action['type'], label: locale === 'en' ? 'Send an email' : locale === 'de' ? 'E-Mail senden' : 'Envoyer un email', desc: locale === 'en' ? 'Creates an email draft from a template — sent after approval in Outreach.' : locale === 'de' ? 'Erstellt einen E-Mail-Entwurf aus einer Vorlage.' : 'Crée un brouillon email depuis un modèle — envoyé après approbation dans Outreach.', fields: ['templateId'] },
    ];
  }, [locale]);
