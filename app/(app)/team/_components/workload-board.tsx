...
export function WorkloadBoard({ workspaceId }: { workspaceId: string }) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 py-2">
      {members.map((m) => (
        <div key={m.userId} className="rounded-xl border border-[#e5e5e0] bg-white p-4 space-y-3">
          <div className="flex items-center gap-3">
          ...
          <div className="space-y-2">
            ...
            <MetricRow icon={Zap} label="Actions NBA" value={m.pendingNba} redIf={m.pendingNba > 5} />
            <div className="flex items-center justify-between text-xs">
              <div className="flex items-center gap-2 text-[#7a7a76]">
