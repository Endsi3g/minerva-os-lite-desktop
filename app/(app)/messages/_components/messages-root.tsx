...
export default function MessagesRoot() {
  const { t } = useLanguage();
  const { activeWorkspace, user: contextUser } = useReach();
    ...
    supabase.auth.getUser().then((authResult: { data: { user: import('@supabase/supabase-js').User | null } }) => {
      ...
      supabase
        ...
        .then((settingsResult: { data: { full_name?: string | null } | null }) => {
          setCurrentUserName(settingsResult.data?.full_name || authUser.email?.split('@')[0] || 'Moi');
        });
    });
  }, []);
  ...
  useEffect(() => {
    if (!activeWorkspace) return;
    const supabase = createClient();
    const channelName = `dm_messages_${activeWorkspace.id}_${selectedConversation}`;
    const channel = supabase
      ...
      .on(
        ...
        (payload: any) => {
          ...
          const belongsToGroup = selectedConversation === 'group' && newMsg.recipientId === null;
          const belongsToDm =
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        // Replace optimistic with real after a short reload (realtime will handle it)
        setMessages((prev) => prev.filter((m) => m.id !== optimisticId));
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="flex h-full w-full overflow-hidden bg-white text-[#26251e]">
      ...
      <div className="w-64 flex-shrink-0 border-r border-[#e5e5e0] bg-[#f4f4f3] flex flex-col">
        ...
        <div className="px-4 pt-5 pb-3">
          <h1 className="text-sm font-bold text-[#26251e] tracking-tight mb-3">
            {t('messages.title')}
          </h1>
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-4">
          ...
          {filteredMembers.length > 0 && (
            <>
              ...
              <p className="text-[10px] font-semibold tracking-wider uppercase text-[#7a7a76] px-2.5 mb-1">
                Membres
              </p>
            </>
          )}
          ...
          {members.length === 0 && (
            <p className="text-[11px] text-[#7a7a76] px-2.5 py-3 text-center">
              Aucun membre actif
            </p>
          )}
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Chat header */}
        <div className="h-14 flex items-center gap-3 px-5 border-b border-[#e5e5e0] shrink-0 bg-white">
          {selectedConversation === 'group' ? (
            <>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={cn('max-w-[70%] flex flex-col', isMe ? 'items-end' : 'items-start')}>
                          ...
                          {/* Timestamp — only on last in group or if showSenderInfo already not shown */}
                          {!showSenderInfo && (
        ...
        {/* Input bar */}
        <div className="px-4 py-3 border-t border-[#e5e5e0] bg-white shrink-0">
          <div className="flex items-center gap-2">
            <Input
              ...
              className="flex-1 h-9 text-sm bg-[#f4f4f3] border-[#e5e5e0] rounded-lg"
              disabled={sending}
            />
            <Button
              onClick={handleSend}
              ...
              size="sm"
              className="h-9 px-3 bg-[#059669] hover:bg-[#047857] text-white rounded-lg shrink-0"
            >
