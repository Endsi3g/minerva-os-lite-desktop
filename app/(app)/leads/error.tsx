...
// Limite le rayon d'un plantage de la liste des leads à cette page, au lieu
// de faire remonter l'erreur jusqu'à global-error.tsx qui blanchit toute
// l'application.
export default function LeadsError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    Sentry.captureException(error);
    reportClientError({
      source: 'leads/error-boundary',
      title: 'Erreur des leads',
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-4 bg-[#fafaf8] px-6 text-center">
      <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#059669]/10">
        <Users2 className="h-6 w-6 text-[#059669]" />
      </div>
      <div className="space-y-1">
        <h2 className="text-sm font-bold text-[#26251e]">Impossible d&apos;afficher les leads</h2>
        <p className="max-w-sm text-xs text-[#7a7a76]">
          Un problème est survenu au chargement de vos prospects. Vos données ne sont pas perdues — réessayez.
        </p>
      </div>
      <button
        onClick={() => reset()}
