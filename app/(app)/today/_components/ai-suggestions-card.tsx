...
export function AiSuggestionsCard() {
  ...
  const getChannelStyle = (channel: string) => {
    switch (channel) {
      ...
      case 'DM': return 'bg-purple-50 text-purple-700';
    ...
    <Card className="border border-[#e5e5e0] bg-white shadow-none">
      ...
      <CardContent className="space-y-4">
        {aiSuggestions.length === 0 ? (
          <p className="text-xs text-[#7a7a76] italic text-center py-4">
            {t('today.ai_suggestions_no_data')}
          </p>
        ) : (
          aiSuggestions.map((sug) => (
            <div key={sug.id} className="flex flex-col gap-2 p-3 rounded-lg border border-[#e5e5e0] bg-secondary/10">
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs font-semibold text-[#26251e] truncate">{sug.leadName}</span>
              ...
              <p className="text-xs font-medium text-[#26251e]/80 leading-relaxed">
                {sug.actionText}
              </p>
              <p className="text-[10px] text-[#7a7a76] italic bg-secondary/40 p-2 rounded">
                💡 {sug.reasoning}
              </p>
              
              <div className="flex items-center justify-between pt-1 mt-1 border-t border-[#e5e5e0]/70">
                <span className="text-[9px] text-[#7a7a76] font-mono truncate max-w-[150px]">
                ...
                <Button 
                  ...
                  className="h-7 px-2 text-[10px] font-semibold gap-1.5 text-[#059669] hover:text-[#059669]/80"
                >
                  {copiedId === sug.id ? (
                    <>
