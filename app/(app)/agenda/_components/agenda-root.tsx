        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-xl bg-[#f54e00]/10 flex items-center justify-center">
              <CalendarDays className="h-5 w-5 text-[#f54e00]" />
            </div>
            <div>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected ? 'border-[#f54e00] bg-[#f54e00]/5' : 'border-transparent hover:border-border hover:bg-muted/40',
                    )}
                  >
                    <span className={cn(
                      'text-xs font-semibold h-5 w-5 flex items-center justify-center rounded-full',
                      isToday ? 'bg-[#f54e00] text-white' : 'text-foreground',
                    )}>
                      {date.getDate()}
                    </span>
                    {dayTasks.length > 0 && (
                      <div className="flex gap-0.5 flex-wrap justify-center">
                        {dayTasks.slice(0, 3).map((t, j) => (
                          <span key={j} className={cn('h-1.5 w-1.5 rounded-full', t.category === 'Meeting' ? 'bg-[#f54e00]' : 'bg-[#059669]')} />
                        ))}
                      </div>
                    )}
                  </button>
          ...
          <div className="rounded-xl border border-border bg-card p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="h-7 w-7 flex items-center justify-center rounded-lg bg-[#f54e00]/10 text-[#f54e00] hover:bg-[#f54e00]/20 transition-colors"
              >
              ...
              <div className="space-y-2">
                {selectedTasks.map(t => (
                  <div key={t.id} className="flex items-start gap-2.5 p-2.5 rounded-lg border border-border bg-background">
                    <div className={cn('h-7 w-7 rounded-lg flex items-center justify-center shrink-0', t.category === 'Meeting' ? 'bg-[#f54e00]/10 text-[#f54e00]' : 'bg-[#059669]/10 text-[#059669]')}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => setShowBook(false)}>
          ...
          >
            ...
            <div className="space-y-3">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  autoFocus
                  className="w-full h-9 text-xs border border-[#e5e5e0] rounded-lg px-3 focus:outline-none focus:ring-1 focus:ring-[#f54e00]"
                />
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="space-y-1.5">
                  ...
                  <input type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)} className="w-full h-9 text-xs border border-[#e5e5e0] rounded-lg px-2 focus:outline-none focus:ring-1 focus:ring-[#f54e00]" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-[#7a7a76]">Heure</label>
                  <input type="time" value={time} onChange={e => setTime(e.target.value)} className="w-full h-9 text-xs border border-[#e5e5e0] rounded-lg px-2 focus:outline-none focus:ring-1 focus:ring-[#f54e00]" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-[#7a7a76]">Durée</label>
                  <select value={duration} onChange={e => setDuration(Number(e.target.value))} className="w-full h-9 text-xs border border-[#e5e5e0] rounded-lg px-2 focus:outline-none focus:ring-1 focus:ring-[#f54e00]">
              ...
              <div className="space-y-1.5">
                ...
                <select value={leadId} onChange={e => setLeadId(e.target.value)} className="w-full h-9 text-xs border border-[#e5e5e0] rounded-lg px-2 focus:outline-none focus:ring-1 focus:ring-[#f54e00]">
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={cn(
                        ...
                        opt.value ? 'bg-[#f54e00] border-[#f54e00]' : 'border-[#e5e5e0]',
                      )}>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 h-10 rounded-lg bg-[#f54e00] hover:bg-[#d94400] text-white text-sm font-bold transition-colors disabled:opacity-60"
            >
