            ...
            <div className="p-6 space-y-5">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <h1 className="text-xl font-black text-[#26251e] leading-tight truncate">{lead.name || lead.company}</h1>
                  ...
                  {lead.rating != null && (
                    <div className="flex items-center gap-1 mt-1.5">
                      {[1,2,3,4,5].map(i => (
                        <Star key={i} className={`h-3 w-3 ${i <= Math.round(lead.rating!) ? 'fill-amber-400 text-amber-400' : 'text-[#e5e5e0]'}`} />
                      ))}
                      <span className="text-[11px] font-bold text-[#26251e] ml-1">{lead.rating}</span>
                ...
                {lead.score != null && (
                  <div className="shrink-0 flex flex-col items-center justify-center bg-[#059669]/8 border border-[#059669]/20 rounded-xl px-3 py-2">
                    <span className="text-lg font-black text-[#059669]">{lead.score}</span>
                    <span className="text-[9px] font-bold text-[#059669]/70 uppercase tracking-wider">/100</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {lead.category && (
                  <div className="inline-flex items-center gap-1 bg-[#f4f4f3] border border-[#e5e5e0] rounded-full px-2.5 py-1">
                    <Tag className="h-2.5 w-2.5 text-[#7a7a76]" />
                    <span className="text-[10px] font-bold text-[#26251e]">{lead.category}</span>
                  </div>
                )}
                {lead.city && (
                  <div className="inline-flex items-center gap-1 bg-[#f4f4f3] border border-[#e5e5e0] rounded-full px-2.5 py-1">
                    <MapPin className="h-2.5 w-2.5 text-[#7a7a76]" />
                    <span className="text-[10px] font-semibold text-[#26251e]">{lead.city}</span>
                  </div>
                )}
              </div>
