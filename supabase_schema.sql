...
create policy "Users can manage team invites" on public.team_invites
  for all using (
    exists (
      select 1 from public.workspaces w
      where w.id = public.team_invites.workspace_id
        and (w.owner_id = auth.uid() or exists (
          select 1 from public.team_members tm
          where tm.workspace_id = w.id and tm.member_user_id = auth.uid() and tm.status = 'active' and tm.role = 'admin'
        ))
    )
  );
...
create policy "Workspace members can see activities" on public.activities
  for select using (
    exists (
      select 1 from public.workspaces w
      where w.id = public.activities.workspace_id
        and (w.owner_id = auth.uid() or exists (
          select 1 from public.team_members tm
          where tm.workspace_id = w.id and tm.member_user_id = auth.uid() and tm.status = 'active'
        ))
