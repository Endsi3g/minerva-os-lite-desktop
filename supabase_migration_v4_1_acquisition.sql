...
UPDATE leads
SET lead_source_type = 'osm'
WHERE (
    source ILIKE '%maps%'
    OR source ILIKE '%osm%'
    OR source ILIKE '%google maps%'
    OR source ILIKE '%places%'
  )
  AND lead_source_type = 'manual';
UPDATE leads
SET lead_source_type = 'csv'
WHERE (
    source ILIKE '%csv%'
    OR source ILIKE '%import%'
  )
  AND lead_source_type = 'manual';
...
CREATE POLICY "Users manage lead events in their workspace" ON lead_events FOR ALL USING (
  workspace_id IN (
    SELECT id
    FROM workspaces
